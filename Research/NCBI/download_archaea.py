...
ssu_rrna_profile = SeqsProfile(SeqsProfileInfo.load_from_dict({
    'name': 'SSU_rRNA_archaea',
    'path': 'SSU_rRNA_archaea.cm',
    'type': 'rna',
    'weight': 1.0,
    'method': 'infernal'
}))
...
for _, row in arch_df_raref.iterrows():
    ...
    fna_seq = [os.path.join(db_dir, ac+"_genomic.fna")]  # archaea
    ...
    try:
        with open(fna_seq[0], 'wb') as gfna_f:  # archaea
            gfna_f.write(zlib.decompress(urllib.request.urlopen(ftp_prefix+"_genomic.fna.gz").read(), 15+32))  # archaea
...
with open("processed_archaea.json", "w") as processed_ac_f:
